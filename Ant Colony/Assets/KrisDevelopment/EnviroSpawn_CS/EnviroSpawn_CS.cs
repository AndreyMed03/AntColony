...
namespace KrisDevelopment.EnvSpawn
{
	...
	public class EnviroSpawn_CS : MonoBehaviour
	{
			...
			if (raycastPositions != null)
				...
				else
				{
					...
					for (int r = 0; r < population; r++)
					{
                        ...
                        {
                            // Если луч не пересек объект или сработал Avoid Mask, пропускаем размещение
                        ...
                        else if (bitmask != null)
                        {
                            ...
                            {
                                // Генерируем новую случайную позицию, если Avoid Mask препятствует размещению
        ...
        {
            // Вычисляем начальную точку луча
            ...
            if (Physics.Raycast(rayOrigin, Vector3.down, out hit, Mathf.Infinity, ~(ignoreMask)))
            {
                ...
                // Проверяем Avoid Mask — если слой объекта совпадает, пропускаем размещение
                if ((avoidMask.value & (1 << hit.transform.gameObject.layer)) != 0)
                {
                    ...
                    bitmask?.Set(r, 0); // Деактивируем текущую позицию
                    return false;
                }

                // Если Avoid Mask не сработал, активируем объект
                bitmask?.Set(r, 1);
                return true;
            }

            // Если луч ничего не пересек, деактивируем позицию
            bitmask?.Set(r, 0);
            return false;
        }
        ...
        private Vector3 GenerateRandomRaycast()
        {
            ...
            // Генерация случайной позиции в пределах размеров
            float scaledX = dimensions.x * transform.localScale.x;
            float scaledZ = dimensions.y * transform.localScale.z;



            // Случайные позиции по осям X и Z в пределах размеров спавнера
            Vector3 rayPos = new Vector3(
                Random.Range(-scaledX / 2, scaledX / 2),  // Генерация случайной координаты по X
                0,  // Позиция по Y остаётся нулевой, так как объекты генерируются на плоскости
                Random.Range(-scaledZ / 2, scaledZ / 2)   // Генерация случайной координаты по Z
            );
